...
def detect_method1(m, draw_lines=True, draw_fit_lines=True):
    ...
    savgol = signal.savgol_filter(
        sig, window_length=FILTER_WINDOW, polyorder=3, deriv=0
    )
    if draw_fit_lines:
        plt.plot(fid, savgol, "-.")
    ...
    if np.count_nonzero(ix_mask):
        ...
        plt.plot(fid, sig * ix_mask, "-m", linewidth=3.0)
...
def detect_method2(m):
    ...
    plt.plot(TIMES, mmean - SIGMA * mstd, "-r", linewidth=0.7)
...
def detect_method2_full(m, draw_lines=True, draw_fit_lines=True):
    ...
    for it in sliding_window(
        range(m.shape[0]), size=WINDOW_SIZE, step=WINDOW_SIZE, fillvalue=-1
    ):
        ix = list(it)
